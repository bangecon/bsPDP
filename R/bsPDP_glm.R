##' Bootstrapped partial dependence plots for the \code{glm} estimator
##'
##' Evaluates a Generalized Linear Models for \code{n_boot} bootstrap iterations to obtain confidence bands for the estimated PDP.
##'
...
##' @param ... additional arguments for the \code{glm} function (e.g. \code{family}).
...
bsPDP.glm <-
  function(formula,
    ...
    for (i in 1:n_boot) {
      ...
      if (test == TRUE) {
        newdf.test <-
          newdata[sample(nrow(newdata), size = N),-which(names(newdata) %in% variable)]
        xp.test <-
        ...
        xp.test$yhat <-
          predict(model, type = 'response', newdata = xp.test)
        xp.test <- cbind(xp.test[, variable], xp.test[, 'yhat'])
      ...
      newdf <-
        df[sample(nrow(df), size = N),-which(names(df) %in% variable)]
      xp <-
        cbind(as.data.frame(grid %x% matrix(1, nrow(newdf))),
              newdf[rep(seq_len(nrow(newdf)), length(grid)), ])
      colnames(xp)[1] <- variable
      xp$yhat <-
        predict(model, type = 'response', newdata = xp)
      xp <- cbind(xp[, variable], xp[, 'yhat'])
    ...
    if (test == TRUE) {
      ...
      pdpHat.test <- rowMeans2(pdps.test[, 2:(1 + n_boot)])
      pdpSd.test <- rowSds(pdps.test[, 2:(1 + n_boot)])
    } else{
    ...
    pdpHat <- rowMeans2(pdps[, 2:(1 + n_boot)])
    pdpSd <- rowSds(pdps[, 2:(1 + n_boot)])
